import "../styles/profile.css";

const Profile = () => {
  return (
    <div className="profile-container">
      <svg className="heading">
        <text x="0%" y="50%" dy=".55em">
          Welcome.
        </text>
      </svg>
      <p className="profile">
        As a full stack developer, my skill set covers HTML, CSS, JavaScript,
        React, Redux, and Ruby on Rails. This expertise enables me to create
        visually appealing and responsive web interfaces while also implementing
        robust back-end functionalities.
        <br />
        <br />I excel at delivering comprehensive web solutions that cater to
        both user experience and business requirements. I am also passionate
        about cutting-edge, pixel-perfect, beautiful interfaces and intuitively
        implemented UX.
      </p>
    </div>
  );
};

export default Profile;
